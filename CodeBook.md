...
_Step 1_: Read in *features.txt* through *read.table* command; extract only the **mean()** and **std()** variables using *grep* command. To exclude undesired variables **meanFreq()** and **angle(...Mean)**, make sure to set **fixed=TRUE** in *grep* command.
